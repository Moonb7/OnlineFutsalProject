...
router.get("/users/userplayer", authMiddleware, async (req, res, next) => {
  try {
...
router.delete("/users/userplayer", authMiddleware, async (req, res, next) => {
  try {
    const userId = req.user.userId;
    const { playerId, upgrade, count: count } = req.body;
    ...
    if (!userPlayer)
      throw new BadRequestError("해당 선수가 인벤토리에 없습니다");

    if (count > userPlayer.count)
      throw new BadRequestError("판매 할 선수가 부족합니다");
    ...
    // 0 ~ 4 : 1 ~ 5
    const sellPlayerPrice =
      player.tiers.salePrice * (userPlayer.upgrade + 1) * userPlayer.count;
